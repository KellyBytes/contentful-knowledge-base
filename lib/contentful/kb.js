...
import { stripLinkFieldsForList, stripLinkFieldsForDetail } from './circular';
...
const fetchArticles = async () => {
  ...
  // return response.items;
  return response.items.map(stripLinkFieldsForList);
};
...
const fetchArticlesByCategory = async categorySlug => {
  ...
  return sortArticles(response.items.map(stripLinkFieldsForList));
};
...
const fetchLatestArticles = async () => {
  ...
  const response = await client.getEntries({
    content_type: 'article',
    order: ['-fields.lastReviewed', '-sys.createdAt'],
    limit: 4,
    include: 2,
  });
...
const fetchArticle = async (slug, preview = false) => {
  ...
  // return response.items[0] ?? null;
  return stripLinkFieldsForDetail(response.items[0] ?? null);
};
